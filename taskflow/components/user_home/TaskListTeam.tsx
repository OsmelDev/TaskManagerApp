...
const TaskListTeam = ({
  ...
  const { tasks, saveTasksTeam } = useTeamStore();
  ...
  const { handleDragEndTeam, handleDragEnd } = useTaskDragAndDrop(tasks);
  ...
  return (
    <div className="flex-1 p-4 rounded-3xl bg-accent-foreground/20 overflow-hidden h-screen md:h-auto">
      ...
      <ScrollArea className="h-[calc(100vh-100px)] md:h-[calc(100vh-150px)]  ">
        <DragDropContext onDragEnd={handleDragEndTeam}>
          <Droppable droppableId="tasksTeam">
            {(provided, snapshot) => (
              <div
